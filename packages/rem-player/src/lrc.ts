/*
 * @Author: kasuie
 * @Date: 2024-08-19 11:10:00
 * @LastEditors: kasuie
 * @LastEditTime: 2024-08-19 11:30:25
 * @Description:
 */
import tplLrc from "../template/lrc.art";
import { LrcOptions } from "./types";
      ...
      for (let i = 0; i < this.current.length; i++) {
        if (
          ...
          this.container
            ...
            [i].classList.add("aplayer-lrc-current");
        }
      }
    }
  }
      ...
      for (let i = 0; i < lyricLen; i++) {
        ...
        if (lrcTimes) {
          ...
          for (let j = 0; j < timeLen; j++) {
            const oneTime: any = /\[(\d{2}):(\d{2})(\.(\d{2,3}))?]/.exec(
              lrcTimes[j]
            );
