/*
 * @Author: kasuie
 * @Date: 2024-08-19 09:32:41
 * @LastEditors: kasuie
 * @LastEditTime: 2024-08-19 14:37:49
 * @Description:
 */
...
import { IOptions, Options } from "./types";
import { randomOrder, isMobile, secondToTime } from "./utils";
import Bar from "./bar";
...
class Player {
  ...
  storage: Storage;
  bar: Bar;
  disableTimeupdate: boolean | undefined;
  audio: any;
  list: any;
  timer: any;
  noticeTime: number | undefined;
  hls: any;
  ...
  initAudio() {
    ...
    for (let i = 0; i < this.events.audioEvents.length; i++) {
      this.audio.addEventListener(this.events.audioEvents[i], (e: any) => {
  ...
  setAudio(audio: any) {
    ...
    if (this.options.customAudioType && this.options.customAudioType[type]) {
      if (
        Object.prototype.toString.call(this.options.customAudioType[type]) ===
        "[object Function]"
      ) {
      ...
      if (type === "hls") {
        if (window && window.Hls && window.Hls.isSupported()) {
   ...
   * Set volume
   */
  volume(percentage?: number | string, nostorage?: boolean) {
    percentage =
  ...
  notice(text: string, time = 2000, opacity = 0.8) {
