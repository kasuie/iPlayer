/*
 * @Author: kasuie
 * @Date: 2024-08-19 14:06:12
 * @LastEditors: kasuie
 * @LastEditTime: 2024-08-19 14:11:21
 * @Description:
 */
import tplListItem from "../template/list-item.art";
// import smoothScroll from "smoothscroll";
import Player from "./player";
import { randomOrder, secondToTime } from "./utils";
...
class List {
  ...
  bindEvents() {
    this.player.template.list.addEventListener(
      "click",
      (e: { target: { tagName: string; parentElement: any } }) => {
        let target;
        ...
        const audioIndex =
          parseInt(
            target.getElementsByClassName("aplayer-list-index")[0].innerHTML
          ) - 1;
  ...
  add(audios: any[]) {
    ...
    ].style.backgroundColor = this.player.options.theme;
  ...
  switch(index: number) {
    ...
    if (typeof index !== "undefined" && this.audios[index]) {
      ...
      const light =
        this.player.container.getElementsByClassName("aplayer-list-light")[0];
      if (light) {
      ...
      if (this.player.duration !== 1) {
        ...
        this.player.template.dtime.innerHTML = secondToTime(
